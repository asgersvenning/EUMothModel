...
def confusion_matrix(model, batch, predictions, labels, step, writer, tround="val"):
    for level in range(3):
        ...
        true = np.array(labels[level])
        pcls = np.array(predictions[level])
        ...
        plt.figure(figsize=(figwidth, figwidth), dpi=max(100, figwidth*50))
        plt.imshow(cm)
        plt.colorbar()
        plt.tight_layout()
